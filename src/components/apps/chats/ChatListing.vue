...
<template>
    ...
    <perfect-scrollbar class="lgScroll chatlist">
        <v-list class="theme-list">
            ...
            >
                ...
                <!---Subtitle-->
                <v-sheet v-if="chat.chatHistory.slice(-1)[0].type == 'img'" class="bg-transparent">
                    <small class="textSecondary text-subtitle-2">Sent a Photo</small>
                </v-sheet>
                <div class="text-subtitle-2 textSecondary mt-1 text-truncate w-100" v-else>
                    {{ chat.chatHistory.slice(-1)[0].msg }}
                </div>
                <!---Last seen--->
                <template v-slot:append>
                    <div class="d-flex flex-column text-right w-25">
                        <small class="textSecondary text-subtitle-2">
                            {{
