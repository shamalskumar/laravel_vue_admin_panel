        ...
        <v-card style="height: auto" class="overflow-auto">
            <v-container>
                <v-card-title class="pa-5">
                    <span class="text-h5">Upload New Signature Image</span>
                </v-card-title>
                <v-card-text>
                    <v-row>
                        ...
                        <v-col cols="12" v-if="signatureImage">
                            <img :src="signatureImage" alt="Selected Signature Image" style="max-width: 100%; height: auto" />
                        </v-col>
                    </v-row>
...
const emit = defineEmits(['update:showDialog', 'signatureImageUploaded']);
const userStore = useUserStore();
const localShowDialog = ref(props.showDialog); // Local state for dialog visibility
const documentType = 'signatureImage';

const signatureImage = ref(''); // URL of the selected image for preview
const signatureImageFile = ref<File | null>(null); // Hold the selected file
...
const handleFileChange = (event: Event) => {
    ...
    if (file) {
        ...
        signatureImage.value = URL.createObjectURL(file); // Show selected image
    }
};
...
const saveData = async () => {
    if (signatureImageFile.value) {
        await convertAndUploadImage(signatureImageFile.value, documentType); // Convert and upload the image
    }
...
const uploadImageToServer = async (blob: Blob, imageType: string) => {
    ...
    formData.append('signature_image', blob, filename);

    if (token && userStore.userData?.id) {
        try {
            // Assuming apiService.updatesignatureImage handles the FormData correctly
            const response = await apiService.updatesignatureImage(token, formData, userStore.userData.id);

            // Emit event after successful upload to notify parent
            emit('signatureImageUploaded'); // Notify parent after image is uploaded
        } catch (error) {
