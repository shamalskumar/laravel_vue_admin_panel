        ...
        <v-card style="height: auto" class="overflow-auto">
            <v-container>
                <v-card-title class="pa-5">
                    <span class="text-h5">Upload New Selfie Image</span>
                </v-card-title>
                <v-card-text>
                    <v-row>
                        ...
                        <v-col cols="12" v-if="selfieImage">
                            <img :src="selfieImage" alt="Selected Selfie Image" style="max-width: 100%; height: auto" />
                        </v-col>
                    </v-row>
...
const emit = defineEmits(['update:showDialog', 'selfieImageUploaded']);
const userStore = useUserStore();
const localShowDialog = ref(props.showDialog); // Local state for dialog visibility
const documentType = 'selfieImage';

const selfieImage = ref(''); // URL of the selected image for preview
const selfieImageFile = ref<File | null>(null); // Hold the selected file
...
const handleFileChange = (event: Event) => {
    ...
    if (file) {
        ...
        selfieImage.value = URL.createObjectURL(file); // Show selected image
    }
};

// Convert and upload the image on save
const saveData = async () => {
    if (selfieImageFile.value) {
        await convertAndUploadImage(selfieImageFile.value, documentType); // Convert and upload the image
    }
...
const uploadImageToServer = async (blob: Blob, imageType: string) => {
    ...
    if (token && userStore.userData?.id) {
        try {
            // Assuming apiService.updateselfieImage handles the FormData correctly
            const response = await apiService.updateselfieImage(token, formData, userStore.userData.id);

            // Emit event after successful upload to notify parent
            emit('selfieImageUploaded'); // Notify parent after image is uploaded
        } catch (error) {
