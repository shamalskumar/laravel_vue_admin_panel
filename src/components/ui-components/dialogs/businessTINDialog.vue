        ...
        <v-card style="height: auto" class="overflow-auto">
            <v-container>
                <v-card-title class="pa-5">
                    <span class="text-h5">Upload New Company T.I.N Image</span>
                </v-card-title>
                <v-card-text>
                    <v-row>
                        <v-col cols="12" sm="12" md="12">
                            <v-file-input
                                v-model="businessTINImageFile"
                                accept="image/*"
                                label="Select Company T.I.N Image*"
                                hide-details
                                variant="outlined"
                                required
                        ...
                        <v-col cols="12" v-if="businessTINImage">
                            <img :src="businessTINImage" alt="Select Company T.I.N Image" style="max-width: 100%; height: auto" />
                        </v-col>
                    </v-row>
...
const emit = defineEmits(['update:showDialog', 'businessTINImageUploaded']);
const userStore = useUserStore();
const localShowDialog = ref(props.showDialog); // Local state for dialog visibility
const documentType = 'businessTINImage';

const businessTINImage = ref(''); // URL of the selected image for preview
const businessTINImageFile = ref<File | null>(null); // Hold the selected file
...
const handleFileChange = (event: Event) => {
    ...
    if (file) {
        businessTINImageFile.value = file;
        businessTINImage.value = URL.createObjectURL(file); // Show selected image
    }
};

// Convert and upload the image on save
const saveData = async () => {
    if (businessTINImageFile.value) {
        await convertAndUploadImage(businessTINImageFile.value, documentType); // Convert and upload the image
    }
...
const uploadImageToServer = async (blob: Blob, imageType: string) => {
    ...
    formData.append('company_tin_image', blob, filename);

    if (token && userStore.userData?.id) {
        try {
            // Assuming apiService.updatebusinessTINImage handles the FormData correctly
            const response = await apiService.updateTINImage(token, formData, userStore.userData.id);

            // Emit event after successful upload to notify parent
            emit('businessTINImageUploaded'); // Notify parent after image is uploaded
        } catch (error) {
