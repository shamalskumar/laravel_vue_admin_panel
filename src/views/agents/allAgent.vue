                                    ...
                                    <tr v-for="item in filteredData" :key="item.id" class="month-item">
                                        ...
                                        <td>
                                            <div style="display: flex; gap: 8px; align-items: center">
                                                ...
                                                <v-btn
                                                    icon
                                                    @click="openSendFundDialog(item.id)"
                                                    style="display: flex; align-items: center; justify-content: center; padding: 0"
                                                >
...
const page = ref({ title: 'All Agents' });
...
const getUsers = async () => {
    try {
        if (token) {
            const fetchedUsers = await apiService.getAllUsers(token, 3);
...
const viewItem = (item: User) => {
    ...
    // Navigate to the "Agent Profile" route
    router.push({ name: 'Agent Profile' });
};

// Open the Send Func dialog
const openSendFundDialog = (userID: number) => {
    formSendFundData.value.send_to = userID;
