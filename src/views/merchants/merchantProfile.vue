...
import UiChildCardKYC from '@/components/shared/UiChildCardKYC.vue';
import {
    UserCircleIcon,
    ReportIcon,
    WalletIcon,
...
import MerchantSubAccountsTab from '@/components/ui-components/tabs/MerchantSubAccountsTab.vue';
...
const items = shallowRef([
    ...
    { tab: 'Reports', icon: ReportIcon, value: '3' },
    { tab: 'Sub-Accounts', icon: HierarchyIcon, value: '4' }
]);

// Filtered items based on condition
const filteredItems = computed(() => {
    return userData.value?.has_sub_accounts === 1
        ? items.value // If condition met, show all items (including 5th tab)
        : items.value.slice(0, 4); // If not, only show first 4 tabs
});
...
async function enableSubAccount() {
    if (token && userStore.userData?.id) {
        try {
            // Update the transfer limit scheme (assuming you have a method for this)
            await apiService.updateSubAccountStatus(1, userStore.userData.id, token);
...
async function disableSubAccount() {
    if (token && userStore.userData?.id) {
        try {
            // Update the transfer limit scheme (assuming you have a method for this)
            await apiService.updateSubAccountStatus(0, userStore.userData.id, token);
            // Fetch the updated schemes again if necessary
...
<template>
            ...
            <v-row class="mt-1">
                    ...
                    <div class="text-center top-spacer">
                        <div class="avatar-border">
                            <v-avatar size="100" class="userImage">
                                <img :src="userData?.selfie_img_url" width="100" alt="Profile Image" />
                            </v-avatar>
                        </div>
                ...
                <v-col cols="12" lg="12" class="d-flex align-center justify-center justify-lg-center order-sm-third">
                    <div class="d-flex flex-column align-center px-10 py-1">
                        ...
                        <div class="d-flex justify-space-between">
                            ...
                            <v-btn
                                v-if="userData?.has_sub_accounts === 0"
                                variant="flat"
                                color="primary"
                                size="small"
                                class="mx-1"
                                @click="enableSubAccount"
                                >Enable Sub-Account</v-btn
                            >
                            <v-btn
                                v-if="userData?.has_sub_accounts === 1"
                                variant="flat"
                                color="error"
                                size="small"
                                class="mx-1"
                                @click="disableSubAccount"
                                >Disable Sub-Account</v-btn
                            >
                        </div>
                    </div>
                </v-col>
                ...
                <v-col md="12" class="order-sm-last">
                    <v-tabs v-model="tab" color="primary" class="profiletab bg-lightprimary">
                        <v-tab v-for="item in filteredItems" :key="item.tab" class="font-weight-regular text-subtitle-1">
    ...
    <v-row class="mt-4">
        ...
        <v-col v-if="tab === 2" cols="12" lg="12">
            <UiChildCardKYC title="KYC Documents">
        ...
        <v-col v-if="tab === 4" cols="12" lg="12">
            <UiChildCard title="Merchant Sub-Account">
                <MerchantSubAccountsTab />
            </UiChildCard>
        </v-col>
    </v-row>
