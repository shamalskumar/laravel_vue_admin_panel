...
import type { City, Country, Income, BillerCategory, Biller } from '@/models/userModel';
...
const page = ref({ title: 'Biller Profile' });
...
const billerCategory = ref<Biller[]>([]); // Store cities from the API with the correct type
...
const formUserData = ref({
    ...
    source_of_income_id: 0,
    personal_tin_no: '',
    biller_name: '',
    biller_category_id: 0
});
...
const openEditUserDialog = async () => {
    ...
    try {
        if (token) {
            ...
            billerCategory.value = fetchedBillerCategory; // Store fetched income sources
        } else {
    ...
    if (userStore) {
        (formUserData.value.first_name = userStore.userData!.first_name || ''),
            ...
            (formUserData.value.gender = userStore.userData!.gender || ''),
            (formUserData.value.address = userStore.userData!.address || ''),
            (formUserData.value.personal_tin_no = userStore.userData!.personal_tin_no || ''),
            (formUserData.value.country = userStore.userData!.city.country_id),
            (formUserData.value.source_of_income_id = userStore.userData!.source_of_income_id),
            (formUserData.value.city_id = userStore.userData!.city_id);
        formUserData.value.biller_name = userStore.userData!.biller.biller_name;
        formUserData.value.biller_category_id = userStore.userData!.biller.biller_category_id;
    }
};
...
const closeProfileDialog = () => {
    ...
    (formUserData.value.city_id = 0),
        ...
        (formUserData.value.gender = ''),
        (formUserData.value.address = ''),
        (formUserData.value.country = 0), // This will hold the selected country ID or object
        (formUserData.value.source_of_income_id = 0),
        (formUserData.value.personal_tin_no = '');
    formUserData.value.biller_category_id = 0;
    formUserData.value.biller_name = '';
};
...
const submitForm = async () => {
    try {
        ...
        if (userStore.userData?.id) {
            ...
            await apiService.patchBiller(token, userStore.userData?.biller.id, formUserData.value);
...
const enableAccount = async () => {
    ...
    if (token && userStore.userData?.id) {
        await apiService.enableBillerAccount(token, userStore.userData?.biller.id, 1);
        searchUsersByID();
    }
};
...
const disableAccount = async () => {
    ...
    if (token && userStore.userData?.id) {
        await apiService.enableBillerAccount(token, userStore.userData?.biller.id, 0);
        searchUsersByID();
    }
};
                    ...
                    <div class="text-center top-spacer">
                        <div class="avatar-border">
                            <v-avatar size="100" class="userImage">
                                <img
                                    v-if="userData?.user_type_id !== 5"
                                    :src="userData?.profile_pic_img_url"
                                    width="100"
                                    alt="Profile Image"
                                />
                                <img
                                    v-if="userData?.user_type_id === 5"
                                    :src="userData?.biller.biller_img_url"
                                    width="100"
                                    alt="Profile Image"
                                />
                            </v-avatar>
                ...
                <v-col cols="12" lg="12" class="d-flex align-center justify-center justify-lg-center order-sm-third">
                    <div class="d-flex flex-column align-center px-10 py-1">
                        ...
                        <div class="d-flex justify-space-between">
                            ...
                            >
                            <v-btn
                                v-if="userData?.biller.is_active === 0"
                                variant="flat"
                                color="primary"
                                size="small"
                                class="mx-1"
                            ...
                            >
                            <v-btn
                                v-if="userData?.biller.is_active === 1"
                                variant="flat"
                                color="error"
                                size="small"
                                class="mx-1"
        ...
        <v-card class="overflow-auto">
            <v-container>
                ...
                <v-card-text>
                    ...
                    <v-row>
                        <v-col cols="12" sm="12" md="12">
                            <v-text-field label="Edit Buisness Name" v-model="formUserData.biller_name" required></v-text-field>
                        </v-col>
                    </v-row>
                    ...
                    <v-row>
                        <v-col cols="12" sm="12" md="12">
                            <v-select
                                label="Edit Biller Category"
                                v-model="formUserData.biller_category_id"
                                :items="billerCategory"
                                item-value="id"
                    ...
                    <v-row>
                        <v-col cols="12" sm="12" md="12">
                            <v-text-field label="Edit Personal TIN" v-model="formUserData.personal_tin_no" required></v-text-field>
                        </v-col>
                    </v-row>
                    <v-row>
                        <v-col cols="12" sm="12" md="12">
                    ...
                    <v-row>
                        <v-col cols="12" sm="12" md="12">
                            <v-select
                                label="Edit Source of Income"
                                v-model="formUserData.source_of_income_id"
                                :items="incomeSources"
                                item-value="id"
                                item-title="source"
                            ></v-select>
