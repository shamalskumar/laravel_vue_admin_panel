                        ...
                        <template v-slot:default>
                            ...
                            <tbody>
                                <tr v-for="item in filteredData" :key="item.id" class="month-item">
                                    <td>
                                        <h6 class="text-subtitle-1 text-no-wrap mb-1">{{ item.voucher.data.promotion_name }}</h6>
                                    </td>
                                    <td>
...
const page = ref({ title: 'All Redeemed Vouchers' });
...
const getvouchers = async () => {
    try {
        if (token) {
            const fetchedVouchers = await apiService.getRedeemedVoucher(token);
